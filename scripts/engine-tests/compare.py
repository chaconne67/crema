...
(tests failing only in the pruned tree, grouped by error kind) into <out dir>. Failures listed in
known-pruned-failures.txt (kept files' checks of removed features) are counted apart: a new failure
outside that list is what needs looking into.
...
def main(pruned_dir, upstream_dir, out_dir):
    ...
    known = {line.strip() for line in (here / "known-pruned-failures.txt").read_text(encoding="utf-8").splitlines()
    ...
    for node in sorted(set(pruned) | set(upstream)):
        ...
        if p[0] in BAD and u[0] in BAD:
            c["양쪽 실패"] += 1
        elif p[0] in BAD and node in known:
            c["줄인 쪽만 실패 (알려진)"] += 1
        elif p[0] in BAD:
            c["줄인 쪽만 실패 (새로)"] += 1
    ...
    keys = ["줄인 쪽 pass", "줄인 쪽 fail", "줄인 쪽 error", "줄인 쪽 skip", "원본 pass", "원본 fail", "원본 error",
            "원본 skip", "원본 missing", "양쪽 실패", "줄인 쪽만 실패 (알려진)", "줄인 쪽만 실패 (새로)", "원본만 실패"]
    ...
    lines += ["", "## 줄인 쪽만 새로 실패, 원인별", "", "| 분류 | 건수 | 대표 파일 | 오류 |", "|---|---|---|---|"]
